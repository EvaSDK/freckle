...
function logon() {
	...
	setcookie( "freckle-esiee_mysql_usr" );
	setcookie( "freckle-esiee_mysql_pwd" );
	echo "<html><head></head><body><form action='$PHP_SELF'>\n";
	echo "<input type=hidden name=action value=logon_submit>\n";
	echo "Username <input type=text name=username>\n";
	echo "Mot de passe  <input type=password name=password>\n";
	echo "<input type=submit value='Entrer'></form></body></html>\n";
}

function logon_submit() {
	global $username, $password, $PHP_SELF;

	setcookie( "freckle-esiee_mysql_usr", $username );
	setcookie( "freckle-esiee_mysql_pwd", $password );
	echo "<html><head><META HTTP-EQUIV=Refresh CONTENT='0; URL=$PHP_SELF?action=listTables&dbname=$db'></head></html>";
}
...
function viewData( $queryStr ) {
	...
	$pResult = mysql_db_query ( $dbname , $queryStr , $mysqlHandle);
	$errMsg = mysql_error();
...
elseif( $action == "logon_submit" ) {
	...
	while( list($var, $value) = each($_COOKIE) ) {
		if( $var == "freckle-esiee_mysql_usr" ) $USERNAME = $value;
		if( $var == "freckle-esiee_mysql_pwd" ) $PASSWORD = $value;
	}
